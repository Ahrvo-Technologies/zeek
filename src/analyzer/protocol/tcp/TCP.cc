...
packet_analysis::TCP::TCPAnalyzerAdapter* TCP_ApplicationAnalyzer::TCP()
	{
	return tcp ?
		tcp :
		static_cast<packet_analysis::TCP::TCPAnalyzerAdapter*>(Conn()->FindAnalyzer("TCP"));
	}
...
void TCP_ApplicationAnalyzer::Init()
	...
	if ( Parent()->IsAnalyzer("TCP") )
		SetTCP(static_cast<packet_analysis::TCP::TCPAnalyzerAdapter*>(Parent()));
	}
