...
TCPAnalyzerAdapter::TCPAnalyzerAdapter(Connection* conn)
	: packet_analysis::IP::AnalyzerAdapter("TCP", conn)
	{
	// Set a timer to eventually time out this connection.
	ADD_ANALYZER_TIMER(&TCPAnalyzerAdapter::ExpireTimer,
...
analyzer::Analyzer* TCPAnalyzerAdapter::FindChild(analyzer::ID arg_id)
	{
	analyzer::Analyzer* child = packet_analysis::IP::AnalyzerAdapter::FindChild(arg_id);
...
analyzer::Analyzer* TCPAnalyzerAdapter::FindChild(analyzer::Tag arg_tag)
	{
	analyzer::Analyzer* child = packet_analysis::IP::AnalyzerAdapter::FindChild(arg_tag);
...
bool TCPAnalyzerAdapter::RemoveChildAnalyzer(analyzer::ID id)
	{
	auto rval = packet_analysis::IP::AnalyzerAdapter::RemoveChildAnalyzer(id);
...
void TCPAnalyzerAdapter::EnableReassembly()
	{
...
void TCPAnalyzerAdapter::SetReassembler(analyzer::tcp::TCP_Reassembler* rorig,
...
const struct tcphdr* TCPAnalyzerAdapter::ExtractTCP_Header(const u_char*& data,
                                                           int& len, int& caplen)
	{
...
bool TCPAnalyzerAdapter::ValidateChecksum(const IP_Hdr* ip, const struct tcphdr* tp,
...
void TCPAnalyzerAdapter::SetPartialStatus(analyzer::tcp::TCP_Flags flags, bool is_orig)
	{
	if ( is_orig )
		{
...
void TCPAnalyzerAdapter::UpdateInactiveState(
...
void TCPAnalyzerAdapter::UpdateStateMachine(
	double t, analyzer::tcp::TCP_Endpoint* endpoint, analyzer::tcp::TCP_Endpoint* peer,
	uint32_t base_seq, uint32_t ack_seq,
...
void TCPAnalyzerAdapter::GeneratePacketEvent(
...
bool TCPAnalyzerAdapter::DeliverData(double t, const u_char* data, int len, int caplen,
...
void TCPAnalyzerAdapter::CheckRecording(bool need_contents, analyzer::tcp::TCP_Flags flags)
	{
...
void TCPAnalyzerAdapter::CheckPIA_FirstPacket(bool is_orig, const IP_Hdr* ip)
	{
...
uint64_t TCPAnalyzerAdapter::get_relative_seq(const analyzer::tcp::TCP_Endpoint* endpoint,
...
int TCPAnalyzerAdapter::get_segment_len(int payload_len, analyzer::tcp::TCP_Flags flags)
	{
...
void TCPAnalyzerAdapter::DeliverPacket(int len, const u_char* data, bool is_orig,
                                       uint64_t seq, const IP_Hdr* ip, int caplen)
	{
	packet_analysis::IP::AnalyzerAdapter::DeliverPacket(len, data, orig, seq, ip, caplen);
...
void TCPAnalyzerAdapter::DeliverStream(int len, const u_char* data, bool orig)
	{
	Analyzer::DeliverStream(len, data, orig);
	}

void TCPAnalyzerAdapter::Undelivered(uint64_t seq, int len, bool is_orig)
	{
	Analyzer::Undelivered(seq, len, orig);
	}

void TCPAnalyzerAdapter::FlipRoles()
	{
	Analyzer::FlipRoles();
...
int TCPAnalyzerAdapter::ParseTCPOptions(const struct tcphdr* tcp, bool is_orig)
	{
	// Parse TCP options.
...
void TCPAnalyzerAdapter::AttemptTimer(double /* t */)
	{
	if ( ! is_active )
		return;
...
void TCPAnalyzerAdapter::PartialCloseTimer(double /* t */)
	{
	if ( ! is_active )
		return;
...
void TCPAnalyzerAdapter::ExpireTimer(double t)
	{
	if ( ! is_active )
		return;
	...
	ADD_ANALYZER_TIMER(&TCPAnalyzerAdapter::ExpireTimer, t + zeek::detail::tcp_session_timer,
	                   false, zeek::detail::TIMER_TCP_EXPIRE);
	}

void TCPAnalyzerAdapter::ResetTimer(double /* t */)
	{
	if ( ! is_active )
		return;
...
void TCPAnalyzerAdapter::DeleteTimer(double /* t */)
	{
	session_mgr->Remove(Conn());
	}

void TCPAnalyzerAdapter::ConnDeleteTimer(double t)
	{
	Conn()->DeleteTimer(t);
	}

void TCPAnalyzerAdapter::SetContentsFile(unsigned int direction, FilePtr f)
	{
...
FilePtr TCPAnalyzerAdapter::GetContentsFile(unsigned int direction) const
	{
	switch ( direction ) {
	case CONTENTS_NONE:
		return nullptr;
	...
	reporter->Error("bad direction %u in TCPAnalyzerAdapter::GetContentsFile",
	                      direction);
	return nullptr;
	}

void TCPAnalyzerAdapter::ConnectionClosed(analyzer::tcp::TCP_Endpoint* endpoint,
	...
	if ( close_complete )
		...
		if ( zeek::detail::tcp_close_delay != 0.0 )
			ADD_ANALYZER_TIMER(&TCPAnalyzerAdapter::ConnDeleteTimer,
			                   Conn()->LastTime() + zeek::detail::tcp_close_delay, false,
			                   zeek::detail::TIMER_CONN_DELETE);
		else
			ADD_ANALYZER_TIMER(&TCPAnalyzerAdapter::DeleteTimer, Conn()->LastTime(), false,
		...
		if ( endpoint->prev_state == analyzer::tcp::TCP_ENDPOINT_INACTIVE )
			...
			if ( connection_partial_close )
				ADD_ANALYZER_TIMER(&TCPAnalyzerAdapter::PartialCloseTimer,
...
void TCPAnalyzerAdapter::ConnectionFinished(bool half_finished)
	{
	const analyzer::analyzer_list& children(GetChildren());
	LOOP_OVER_CONST_CHILDREN(i)
		// Again, nasty - see TCPAnalyzerAdapter::ConnectionClosed.
		static_cast<analyzer::tcp::TCP_ApplicationAnalyzer*>
...
void TCPAnalyzerAdapter::ConnectionReset()
	{
...
bool TCPAnalyzerAdapter::HadGap(bool is_orig) const
	{
	analyzer::tcp::TCP_Endpoint* endp = is_orig ? orig : resp;
	return endp && endp->HadGap();
	}

void TCPAnalyzerAdapter::AddChildPacketAnalyzer(analyzer::Analyzer* a)
	{
...
bool TCPAnalyzerAdapter::DataPending(analyzer::tcp::TCP_Endpoint* closing_endp)
	{
	if ( Skipping() )
		return false;
...
void TCPAnalyzerAdapter::EndpointEOF(analyzer::tcp::TCP_Reassembler* endp)
	{
	if ( connection_EOF )
...
bool TCPAnalyzerAdapter::IsReuse(double t, const u_char* pkt)
	{
